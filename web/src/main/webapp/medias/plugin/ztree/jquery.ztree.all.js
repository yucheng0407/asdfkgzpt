        ...
        view = {
            ...
            makeDOMNodeLine: function (html, setting, node) {
                if (data.nodeIsParent(setting, node)) {
                    if (node.open) {
                        html.push("<span id='", node.tId, consts.id.SWITCH, "' title='' class='iconfont ", view.makeNodeLineClass(setting, node), "' treeNode", consts.id.SWITCH, ">&#xe86d;</span>");
                    } else {
                        html.push("<span id='", node.tId, consts.id.SWITCH, "' title='' class='iconfont ", view.makeNodeLineClass(setting, node), "' treeNode", consts.id.SWITCH, ">&#xe654;</span>");
                    }
                } else {
            ...
            makeNodeIcoStyle: function (setting, node) {
                ...
                if (!node.isAjaxing) {
                    ...
                    if (window.screen.width > 1200 && !RX.browser.isIE8) {
                        var path = icon.substring(0, icon.lastIndexOf("/"));
                        icon = icon.substring(icon.lastIndexOf("/"), icon.length);
                        icon = path + icon.split(".")[0] + "_big.png";
                        if (RX.cookie("skin")) {
                            if (RX.cookie("skin") === "dark" || RX.cookie("skin") === "gray") {
                                icon = icon.replace('default', RX.cookie("skin"));
                            }
                        }
                        icon && icoStyle.push("background:url(", icon, ") 0 0 no-repeat /16px 16px;");
                    } else {
                        icon && icoStyle.push("background:url(", icon, ") 0 0 no-repeat;");
                    }
            ...
            makeNodeLineClass: function (setting, node) {
                ...
                if (data.nodeIsParent(setting, node)) {
                    //fix me=============初始化展开收缩按钮============================
                    if (node.open) {
                        lineClass.push(consts.folder.OPEN);
                        iconIcon = "icon-open ";
                    } else {
                        lineClass.push(consts.folder.CLOSE);
                        iconIcon = "icon-close ";
                    }
                ...
                return view.makeNodeLineClassEx(node) + iconIcon + lineClass.join('_');
            },
            makeNodeLineClassEx: function (node) {
                return "iconfont " + consts.className.LEVEL + node.level + " " + consts.className.SWITCH + " ";
            ...
            replaceSwitchClass: function (node, obj, newName) {
                if (!obj) return;
                //fix me  点解展开节点时，图标随之变化。
                var tmpName = obj.attr("class");
                var temp = tmpName.split(" ");
                var i = 0;
                for (i; i < temp.length; i++) {
                    if (temp[i] == "icon-close" || temp[i] == "icon-open") {
                        if (newName == consts.folder.OPEN) {
                            obj.html("&#xe86d;");
                            temp[i] = "icon-open";
                            continue;
                        } else if (newName == consts.folder.CLOSE) {
                            temp[i] = "icon-close";
                            obj.html("&#xe654;")
                        }
                    }
