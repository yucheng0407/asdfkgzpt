...
/**
 * 2018-04-09 修改了绑定事件的方法，采用jquery on绑定，使用_laydate的名称空间，方便去除事件
 * 2018-08-21 增加setVal配置，用于调用setValue时的触发函数  by mrq
 * 2018-09-05 修改dateType为date和month时的处理，选择日期之后不需要确认 a.range||n.setValue(n.parse()).remove().done()
 * 2018-09-27 增加adjustDate接口，解决日期手动输入可以超出限制的bug
 * 2018-10-09 max: "2099-12-31"改为 max: "2099-12-31 23:59:59",
 *              调整adjustDate接口相关逻辑，修改type为time时的bug
 *              依赖rx.date.js，使用其中接口RX.parseDateByFmt
 */
;!function () {
    "use strict";
    ...
    C.prototype = [], C.prototype.constructor = C, w.extend = function () {
        ...
        var t, a, i = this, r = (new Date, i.config), o = r.dateTime = r.dateTime || i.systemDate(),
            ...
            //add by mrq 解决日期手动输入可以超出限制的bug
            adjustDate = function (dateStr, e) {
                var nowDate = RX.parseDateByFmt(dateStr, i.config.format);
                nowDate = nowDate && nowDate.getTime();
                var maxDate = i.config.max, minDate = i.config.min;
                if (nowDate) {
                    //最大日期为当前日期，在输入框中可以随便填写日期，内置的只会验证日期合法等所以需要验证
                    if (r.initMax === 0) {
                        //当前日期不能大于最大日期
                        maxDate.hours = 23;
                        maxDate.minutes = 59;
                    ...
                    var flg = false;
                    if (nowDate > RX.parseDateByFmt(new Date(maxDate.year, maxDate.month, maxDate.date, maxDate.hours, maxDate.minutes, maxDate.seconds).Format(i.config.format), i.config.format).getTime()) {
                        //大于最大值
                        e.year = maxDate.year;
                        e.month = maxDate.month;
                        e.date = maxDate.date;
                        e.hours = maxDate.hours;
                        e.minutes = maxDate.minutes;
                        e.seconds = maxDate.seconds;
                        flg = true;
                    } else if (nowDate < RX.parseDateByFmt(new Date(minDate.year, minDate.month, minDate.date, minDate.hours, minDate.minutes, minDate.seconds).Format(i.config.format), i.config.format).getTime()) {
                        //小于最小值
                        e.year = minDate.year;
                        e.month = minDate.month;
                        e.date = minDate.date;
                        e.hours = minDate.hours;
                        e.minutes = minDate.minutes;
                        e.seconds = minDate.seconds;
                        flg = true;
                    }
                    return flg;
                }
        ...
        if (y && h.removeChild(y), h.appendChild(d), "year" === e || "month" === e) w(n.elemMain[t]).addClass("laydate-ym-show"), w(d).find("li").on("click", function () {
            ...
            if (!w(this).hasClass(s)) {
                ...
                "year" === a.type || "month" === a.type ? (w(d).find("." + o).removeClass(o), w(this).addClass(o), "month" === a.type && "year" === e && (n.listYM[t][0] = r, l && (n[["startDate", "endDate"][t]].year = r), n.list("month", t))) : (n.checkDate("limit").calendar(), n.closeList()), n.setBtnStatus(), w(n.footer).find(D).removeClass(s), a.range || n.setValue(n.parse()).remove().done()
            }
        }); else {
        ...
        typeof n.setVal === "function" && n.setVal(e);
        ...
        var e = this, t = e.config, n = function (n, a) {
            $(n[0]).on(t.trigger + "._laydate", function () {
