(function (global, factory) {
    ...
    //FixMe:———start————增加存在判断方法————Zp.20180531
    function exist(value) {
        return exist !== null || typeof exist !== "undefined";
    };
    //FixMe:————end—————增加存在判断方法————Zp.20180531
    ...
    var debug = /Rxvm/.test(toString(noop));
...
// 比如线上环境，关了 debug 模式，为了调试，想强制打开
     ...
     */
    function fatal(msg) {
        throw new Error('[Rxvm fatal]: ' + msg);
    }
...
//
    ...
    var HOOK_SYS_BEFORE_CREATE = '_beforeCreate';
    var HOOK_AFTER_CREATE = 'afterCreate';
    var HOOK_SYS_AFTER_CREATE = '_afterCreate';
    var HOOK_BEFORE_MOUNT = 'beforeMount';
    var HOOK_SYS_BEFORE_MOUNT = '_beforeMount';
    var HOOK_AFTER_MOUNT = 'afterMount';
    var HOOK_SYS_AFTER_MOUNT = '_afterMount';
    var HOOK_BEFORE_UPDATE = 'beforeUpdate';
    var HOOK_SYS_BEFORE_UPDATE = '_beforeUpdate';
    var HOOK_AFTER_UPDATE = 'afterUpdate';
    var HOOK_SYS_AFTER_UPDATE = '_afterUpdate';
    var HOOK_BEFORE_DESTROY = 'beforeDestroy';
    var HOOK_SYS_BEFORE_DESTROY = '_beforeDestroy';
    var HOOK_AFTER_DESTROY = 'afterDestroy';
    var HOOK_SYS_AFTER_DESTROY = '_afterDestroy';
    //FixMe:———start————渲染后置————Zp.20180521
    //初次渲染同afterMount，局部更新同afterUpdate
    var HOOK_AFTER_RENDER = 'afterRender';
    var HOOK_SYS_AFTER_RENDER = '_afterRender';
    //FixMe:————end—————渲染后置————Zp.20180521
    //FixMe:———start————值变更后置————Zp.20180523
    //值变更后置，可用于向外更新（同步）数据
    var HOOK_AFTER_SET = 'afterSetValue';
    var HOOK_SYS_AFTER_SET = '_afterSetValue';

    //FixMe:————end—————值变更后置————Zp.20180523
...
// 缓存编译结果
    ...
    var Observer = function () {
        ...
        Observer.prototype.get = function (keypath, defaultValue) {
            ...
            //FixMe:———start————当keypath不存在时，获取全部数据————Zp.20180531
            if (keypath === null || typeof keypath === "undefined") {
                return this.data;
            }
            //FixMe:————end—————当keypath不存在时，获取全部数据————Zp.20180531
        ...
        Observer.prototype.set = function (keypath, value) {
            ...
            //FixMe:———start————父子组件同步更新参数初始化————Zp.20180625
            var syncForward, syncTag = false, syncFromCid, bindTag = false;
            if (typeof keypath === "string") {
                if (keypath.startWith("sync|")) {
                    if (instance.context.$syncDataTag) {
                        var pathstr = keypath.split(":");
                        var syncstr = pathstr[0].split("|");
                        syncForward = syncstr[1];
                        syncFromCid = syncstr[2];
                        keypath = pathstr[1];
                    } else {
                        return;
                    }
                    syncTag = true;
                } else if (keypath.startWith("bind:")) {
                    keypath = keypath.substring(5);
                    bindTag = true;
                }
            }
            //FixMe:————end—————父子组件同步更新参数初始化————Zp.20180625

            //FixMe:———start————如开启自动数据同步，则关闭原有的组件bind数据功能————Zp.20180625
            if (bindTag && instance.context.$syncDataTag) {
                return;
            }
            //FixMe:————end—————如开启自动数据同步，则关闭原有的组件bind数据功能————Zp.20180625
            ...
            //FixMe:———start————父子组件同步更新辐射————Zp.20180625
            var vm = instance.context,
                syncPath,
                newValue;

            if (typeof keypath === "string" && !bindTag && vm.$syncDataTag) {
                if (!syncForward) {
                    newValue = copy(value, true);
                    if (vm.$children) {
                        syncPath = "sync|child|" + vm.cid + ":";
                        for (var i = 0; i < vm.$children.length; i++) {
                            var cVm = vm.$children[i];
                            if (cVm.$syncDataTag && cVm.$syncKey) {
                                for (var key in cVm.$syncKey) {
                                    var pkey = cVm.$syncKey[key];
                                    if (keypath.startWith(pkey)) {
                                        cVm.set(syncPath + key + keypath.substring(pkey.length), newValue);
                                        break;
                                    }
                                }
                            }
                    ...
                    if (vm.$parent && vm.$parent.$syncDataTag && vm.$syncKey) {
                        syncPath = "sync|parent|" + vm.cid + ":";
                        for (var key in vm.$syncKey) {
                            var pkey = vm.$syncKey[key];
                            if (keypath.startWith(key)) {
                                vm.$parent.set(syncPath + pkey + keypath.substring(key.length), newValue);
                                break;
                            }
                        }
                    }

                } else if (syncForward === "child") {
                    if (vm.$children) {
                        newValue = copy(value, true);
                        syncPath = "sync|child|" + vm.cid + ":";
                        for (var i = 0; i < vm.$children.length; i++) {
                    ...
                    if (vm.$children) {
                        ...
                        for (var i = 0; i < vm.$children.length; i++) {
                            var cVm = vm.$children[i];
                            if (cVm.$syncDataTag && syncFromCid !== cVm.cid && cVm.$syncKey) {
                                for (var key in cVm.$syncKey) {
                                    var pkey = cVm.$syncKey[key];
                                    if (keypath.startWith(pkey)) {
                                        cVm.set(syncPath + key + keypath.substring(pkey.length), newValue);
            ...
            //FixMe:————end—————父子组件同步更新辐射————Zp.20180625

            //FixMe:———start————参数对象字段初始化————Zp.20180628
            if (typeof value === "object") {
                //如何解决默认值的同步
                instance.context._renderDefaultValue(false, keypath, value);
            }
            //FixMe:————end—————参数对象字段初始化————Zp.20180628

            //FixMe:———start————触发值变更后置————Zp.20180523
            if(!syncTag && !bindTag){
                //FixMe:———start————系统声明周期事件————Zp.20180611
                execute(instance.context.$options[HOOK_SYS_AFTER_SET], instance.context);
                //FixMe:————end—————系统声明周期事件————Zp.20180611
                execute(instance.context.$options[HOOK_AFTER_SET], instance.context);
            }
            //FixMe:————end—————触发值变更后置————Zp.20180523
        };
        ...
        //FixMe:———start————扩展方法：移除数组中的所有元素————Zp.20180628
        /**
         * 移除数组中的所有元素
         *
         * @param {string} keypath
         * @return {?boolean} 是否移除成功
         */
        Observer.prototype.removeAll = function (keypath) {
            var list = this.get(keypath);
            if (array(list)) {
                list = [];
                this.set(keypath, list);
        ...
        //FixMe:————end—————扩展方法：移除数组中的所有元素————Zp.20180628
     ...
     *
     * 框架未实现此事件，通过 Rxvm.dom.specialEvents 提供给外部扩展
     *
     * @type {string}
     */
    var TAP = 'tap';
    ...
    //FixMe:———start————触发事件接口————Zp.20180620
    api.trigger = function (element, type, event, context) {
        var emitter = element[EMITTER_KEY];
        if (emitter) {
            emitter.fire(type, new Event(api.createEvent(event, element)));
        }
    }
    //FixMe:————end—————触发事件接口————Zp.20180620
...
// 移动端的 tap 事件可自行在业务层打补丁实现
            ...
            if($el.prop("tagName") === "DIV"){
                ...
                if($inputs.length){
                    ...
                    if($inputs[0].type=="checkbox"){
                        setCheckLabelByInput($inputs[0]);
                    }else if($inputs[0].type=="radio"){
                        setRadioLabelByInput($inputs[0]);
                    }
                }
            }
            //FixMe:———start————input个性处理————Zp.20180709
            CR.updateSpanShow(el, value);
            //FixMe:———end————input个性处理————Zp.20180709
        },
        sync: function sync(el, keypath, instance) {
            // instance.set(keypath, el[RAW_VALUE]);
            //FixMe:———start————DIV个性处理————Zp.20180719
            var $el = $(el);
            if($el.prop("tagName") === "DIV"){
                var value = [];
                ...
                instance.set(keypath, value.join());
            }else{
                instance.set(keypath, el[RAW_VALUE]);
            }
            //FixMe:———end————DIV个性处理————Zp.20180719
        },
    ...
    var selectControl = {
        set: function set$$1(el, keypath, instance) {
            //FixMe:———start————select个性处理————Zp.20180628
            var value = toString(instance.get(keypath));
            // var options = el.options,
            //     selectedIndex = el.selectedIndex;
            // var validValue = CR.renderSelectValue(el,value);
            CR.renderSelectValue(el, value, instance);
            CR.updateSpanShow(el, value);
            CR.updateTitle(el, value);

            // if (selectedIndex >= 0) {
            //     var selectedOption = options[selectedIndex];
            //     if (selectedOption) {
            //         var newValue = isDef(selectedOption[RAW_VALUE]) ? selectedOption[RAW_VALUE] : selectedOption.text;
            //         if (value !== newValue) {
            //             each(options, function (option, index) {
            //                 var optionValue = isDef(option[RAW_VALUE]) ? option[RAW_VALUE] : option.text;
            //                 if (optionValue === newValue) {
            //                     el.selectedIndex = index;
            //                     CR.updateSpanShow(el, value);
            //                     CR.updateTitle(el, value);
            //                     CR.renderSelectValue(el);
            //                     return FALSE;
            //                 }
            //             });
            //         }
            //     } else {
            //         CR.updateSpanShow(el, "");
            //         CR.updateTitle(el, "");
            //         CR.renderSelectValue(el, "");
            //     }
            // }
            //FixMe:————end—————select个性处理————Zp.20180620
        },
        sync: function sync(el, keypath, instance) {
    ...
    var radioControl = {
        set: function set$$1(el, keypath, instance) {
            //FixMe:———start————radio个性处理————Zp.20180620
            el.checked = el[RAW_VALUE] === toString(instance.get(keypath));
            CR.renderRadioValue(el);
            //FixMe:————end—————radio个性处理————Zp.20180620
        },
        sync: function sync(el, keypath, instance) {
            if (el.checked) {
                instance.set(keypath, el[RAW_VALUE]);
    ...
    var checkboxControl = {
        set: function set$$1(el, keypath, instance) {
            ...
            //FixMe:———start————checkbox个性处理————Zp.20180620
            var value = instance.get(keypath, '');
            if (!exist(el[RAW_VALUE])) {
                el.checked = boolean(value) ? value : !!value;
            } else {
                if ((',' + value + ',').indexOf(',' + el[RAW_VALUE] + ',') > -1) {
                    el.checked = true;
                } else {
                    el.checked = false;
                }
            }
            CR.renderCheckboxValue(el);
            //FixMe:————end—————checkbox个性处理————Zp.20180620
        },
        sync: function sync(el, keypath, instance) {
            // var value = instance.get(keypath);
            // if (array(value)) {
            ...
            // }
            //FixMe:———start————checkbox个性处理————Zp.20180531
            var value = instance.get(keypath, '');
            if (!exist(el[RAW_VALUE] === null)) {
                instance.set(keypath, el.checked);
            } else {
                var valueArr = value.split(","), vLength = valueArr.length, inNum = -1;
                for (var i = 0; i < vLength; i++) {
                    if (el[RAW_VALUE].toString() === valueArr[i]) {
                        inNum = i;
                        break;
                    }
                }
                if (inNum > -1) {
                    if (!el.checked) {
                        valueArr.splice(inNum, 1);
                    }
                } else {
                    if (el.checked) {
                        valueArr.push(el[RAW_VALUE].toString());
                    }
                }
                instance.set(keypath, valueArr.join());
            }
            //FixMe:————end—————checkbox个性处理————Zp.20180531
        },
    ...
    var model = function (_ref) {
        ...
        if (keypath) {
            ...
            //FixMe:———start————插入待渲染路径————Zp.20180613
            // console.log("push path:"+keypath);
            instance._renderPath.push(keypath);
            //FixMe:————end—————插入待渲染路径————Zp.20180613
            ...
            //FixMe:———start————增加配置标记————Zp.20180525
            var $el = $(el);
            $el.attr("rx-path", keypath);
            $el.attr("rx-cid", instance.cid);
            //FixMe:————end—————增加配置标记————Zp.20180525

            //FixMe:———start————同步dom个性config————Zp.20180910
            var rxConfig = $el.attr("rx-config");
            if(rxConfig){
                var config = instance.$options.config,
                    value = {};
                try{
                    value = RX.parseJson(rxConfig);
                }catch(e){
                    value = {}
                }
                config[keypath] = instance._getCombineConfig(value, config[keypath] || {});
            }
            //FixMe:————end—————同步dom个性config————Zp.20180910
    ...
    var binding = function (_ref) {
        ...
        //FixMe:———start————识别组件绑定————Zp.20180522
        if (component) {
            component.$syncKey = component.$syncKey || {};
            component.$syncKey[node.modifier] = node[RAW_VALUE];
        }
        //FixMe:————end—————识别组件绑定————Zp.20180522
        ...
        if (keypath) {
            var set = function (value) {
                ...
                if (component) {
                    //FixMe:———start————binding的set额外增加标记————Zp.20180522
                    component.set("bind:" + name, value);
                    //FixMe:————end—————binding的set额外增加标记————Zp.20180522
                } else {
...
// 组件是否存在某个 slot
    ...
    //FixMe:———start————声明获取唯一id方法————Zp.20180523
    var uniqueIndex = 0;
    var getUniqueId = function () {
        return ++uniqueIndex;
    }
    //FixMe:————end—————声明获取唯一id方法————Zp.20180523

    //FixMe:———start————声明整合控件options的方法————Zp.20180612
    function combineWidgetOptions(widgetOptions, options) {
        widgetOptions = widgetOptions || {};
        options = options || {};
        var initOptions = widgetOptions["initOptions"];
        if (initOptions && typeof initOptions === "function") {
            return $.extend(true, {}, initOptions(widgetOptions, options));
        } else {
            return $.extend(true, {}, widgetOptions, options);
        }
    }

    //FixMe:————end—————声明整合控件options的方法————Zp.20180612

    var Rxvm = function () {
        function Rxvm(options) {
            classCallCheck(this, Rxvm);
            ...
            //FixMe:———start————启动widget的options初始化————Zp.20180610
            options = combineWidgetOptions(options.widget, options);
            //FixMe:————end—————启动widget的options初始化————Zp.20180610

            //FixMe:———start————config初始化————Zp.20180523
            options.config = options.config || {};
            //FixMe:————end—————config初始化————Zp.20180523

            //FixMe:———start————注册唯一cid————Zp.20180523
            instance.cid = "c" + getUniqueId();
            //FixMe:————end—————注册唯一cid————Zp.20180523

            //FixMe:———start————注册实例————Zp.20180604
            Rxvm._backupViewModel(instance);
            //FixMe:————end—————注册实例————Zp.20180604

            // 如果不绑着，其他方法调不到钩子
            instance.$options = options;

            //FixMe:———start————历史数据初始化————Zp.20180611
            instance.$historyData = {};
            //FixMe:————end—————历史数据初始化————Zp.20180611

            //FixMe:———start————文件组件池初始化————Zp.20180724
            instance.$fileVmPool = {};
            //FixMe:————end—————文件组件池初始化————Zp.20180724

            //FixMe:———start————系统声明周期事件————Zp.20180611
            execute(options[HOOK_SYS_BEFORE_CREATE], instance, options);
            //FixMe:————end—————系统声明周期事件————Zp.20180611
            execute(options[HOOK_BEFORE_CREATE], instance, options);

            //FixMe:———start————待渲染路径初始化————Zp.20180613
            instance._renderPath = [];
            //FixMe:————end—————待渲染路径初始化————Zp.20180613
            ...
            //FixMe:———start————同步数据标记初始化————Zp.20180613
            var _syncData = _options.settings && _options.settings.syncData;
            if (typeof _syncData !== "boolean") {
                if (_syncData === "false") {
                    _syncData = false;
                } else {
                    _syncData = true;
            ...
            this.$syncDataTag = _syncData;
            //FixMe:————end—————同步数据标记初始化————Zp.20180613
            ...
            //FixMe:———start————调整模板获取逻辑————Zp.20180621
            //容器内已存在html，可能包含模板script代码
            var elInnerHtml;
            ...
            var containerTplTag = false,selectorTplTag = false;

            //将容器中的script赋予空模板
            if (!template && elInnerHtml) {
                var fragArr1 = elInnerHtml.split("<script");
                var fragArr2,scriptHtml;
                if (fragArr1.length > 1) {
                     fragArr2 = fragArr1[1].split("</script");
                    if (fragArr2.length > 1) {
                         scriptHtml = fragArr2[0].substring(fragArr2[0].indexOf(">") + 1);
                        _options.template = template = scriptHtml;
                        containerTplTag = true;
                    }
                } else {  //补丁，IE下会将script标签自动转换为大写
                     fragArr1 = elInnerHtml.split("<SCRIPT");
                    if (fragArr1.length > 1) {
                         fragArr2 = fragArr1[1].split("</SCRIPT");
                        if (fragArr2.length > 1) {
                             scriptHtml = fragArr2[0].substring(fragArr2[0].indexOf(">") + 1);
            ...
            //template不存在，则设为默认模板
            if (!template && _options.defaultTemplate){
            ...
            if (string(template)) {
                ...
                //选择器模板获取
                if (!containerTplTag && selector.test(template)) {
                    template = api.html(api.find(template));
                    selectorTplTag = true;
                }

                if(template && !selectorTplTag && !containerTplTag && template.startWith("loadTpl:")){
                    template =RX.loadTpl(template.substring(8));
                }
            ...
            //子组件模板快速获取
            if (string(template) && components) {
                for (var childKey in components) {
                    if (components.hasOwnProperty(childKey)) {
                        var cmpt = components[childKey];
                        var fragArr1 = template.split("<" + childKey), l1 = fragArr1.length,
                            cmptTplHtml = "";
                        if (fragArr1.length > 1) {
                            for (var i = 1; i < l1; i++) {
                                var frag1 = fragArr1[i],
                                    end1 = frag1.indexOf("/>"),
                                    end2 = frag1.indexOf(">"),
                                    end3 = frag1.indexOf("</" + childKey);
                                if (end2 > -1 && end3 > -1) {
                                    if (!cmptTplHtml) {
                                        cmptTplHtml = frag1.substring(end2 + 1, end3);
                                    }
                                    var frag11 = frag1.substring(end3 + 2 + childKey.length);
                                    frag11 = frag11.substring(frag11.indexOf(">") + 1);
                                    fragArr1[i] = frag1.substring(0, end2) + "/>" + frag11;
                                }
                            }
                            template = fragArr1.join("<" + childKey);
                        }
                        if (cmpt && cmptTplHtml && !cmpt.template) {
                            cmpt.template = cmptTplHtml;
                        }
                        cmptTplHtml = "";
                    }
                }
            ...
            //FixMe:————end—————调整模板获取逻辑————Zp.20180621
            ...
            //FixMe:———start————实现setting.getData获取值逻辑————Zp.20180528
            //FixMe:———start————系统声明周期事件————Zp.20180611
            execute(options[HOOK_SYS_AFTER_CREATE], instance);
            //FixMe:————end—————系统声明周期事件————Zp.20180611
            ...
            var _getData = _options.settings && _options.settings.getData;
            if (_getData) {
                if (_getData.url) {
                    var defAjaxObj = {
                        type: "post",
                    ...
                    ajaxObj.success = function (ar) {
                        var _success = _getData.success;
                        if (typeof _success === "function") {
                            setSource(_success(ar, _getData.defaultData));
                        } else if (ar._TYPE === "ajaxReturn") {
                            if (ar.success) {
                                setSource(ar.data);
                            } else if (ar.msg) {
                                if (RX && RX.alert) {
                    ...
                    setSource(_getData.defaultData);
                    drawData();
                }
            } else {
                drawData();
            ...
            function drawData() {
                ...
                //FixMe:———start————触发默认值渲染————Zp.20180525
                // instance._renderDefaultValue(true);
                //FixMe:————end—————触发默认值渲染————Zp.20180525

                // //FixMe:———start————初始化状态————Zp.20180529
                // if (_options.state) {
                //     instance.setState(_options.state, null, false);
                // } else {
                //     _options.state = {_defaultEnable: true};
                // }
                // //FixMe:————end—————初始化状态————Zp.20180525

                if (template) {
                    // 确保组件根元素有且只有一个
                    template = Rxvm.compile(template);
                ...
                //FixMe:———start————再次触发默认值渲染————Zp.20180910
                //同步个性dom配置默认值，且为undefined值字段赋初值（默认值 || ""）
                instance._renderDefaultValue(false);
                //FixMe:————end—————再次触发默认值渲染————Zp.20180910

                //FixMe:———start————同步子组件初始数据————Zp.20181102
                if(instance.$children) {
                    $.each(instance.$children, function(i,c){
                        if(c.$syncDataTag){
                            c._syncData();
                        }
                    })
                }
                //FixMe:————end—————同步子组件初始数据————Zp.20181102
            }

            //FixMe:————end—————实现setting.getData获取值逻辑————Zp.20180528
        }

        Rxvm.prototype.validate = function (props) {
            var $propTypes = this.$propTypes;

            return $propTypes ? Rxvm.validate(props, $propTypes) : props;
        };
        ...
        Rxvm.prototype.addComputed = function (keypath, computed) {
        ...
        Rxvm.prototype.get = function (keypath, defaultValue) {
            //FixMe:————start—————get富文本字段时，更新富文本控件值————Zp.20180710
            if(this.$richPool && this.$richPool[keypath]){
                this._syncRichArea(keypath);
            }
            //FixMe:————end—————get富文本字段时，更新富文本控件值————Zp.20180710
        ...
        Rxvm.prototype.set = function (keypath, value) {
        ...
        Rxvm.prototype.on = function (type, listener) {
        ...
        Rxvm.prototype.once = function (type, listener) {
        ...
        Rxvm.prototype.off = function (type, listener) {
        ...
        Rxvm.prototype.fire = function (type, data, downward) {
        ...
        Rxvm.prototype.watch = function (keypath, watcher, sync) {
        ...
        Rxvm.prototype.watchOnce = function (keypath, watcher, sync) {
        ...
        Rxvm.prototype.unwatch = function (keypath, watcher) {
        ...
        Rxvm.prototype.forceUpdate = function () {
        ...
        Rxvm.prototype.render = function () {
        ...
        Rxvm.prototype.updateView = function (newNode, oldNode) {
            ...
            //FixMe:———start————自适应优化参数注册————Zp.20180702
            var needResize = $options.settings && $options.settings.needResize;
            needResize = typeof needResize === "boolean" ? needResize : !(needResize === "false" || instance.$parent);
            //FixMe:————end—————自适应优化参数注册————Zp.20180702
            ...
            if ($node) {
                //FixMe:———start————系统声明周期事件————Zp.20180611
                execute($options[HOOK_SYS_BEFORE_UPDATE], instance);
                //FixMe:————end—————系统声明周期事件————Zp.20180611
                ...
                //FixMe:———start————系统声明周期事件————Zp.20180611
                execute($options[HOOK_SYS_BEFORE_MOUNT], instance);
                //FixMe:————end—————系统声明周期事件————Zp.20180611
                ...
                //FixMe:———start————加载优化：隐藏————Zp.20180702
                if (!instance.$parent) {
                    RX.page.showLoading(instance.cid);
                }
                //FixMe:————end—————加载优化：隐藏————Zp.20180611
            }
            ...
            append(function () {
                if (instance.$node) {
                    //FixMe:———start————加载优化：显示————Zp.20180702
                    if (!instance.$parent) {
                        RX.page.hideLoading(instance.cid);
                    }
                    //FixMe:————end—————加载优化：显示————Zp.20180611
                    //FixMe:———start————触发配置渲染————Zp.20180525
                    // var renderPath = instance._renderPath;
                    // console.log("path:" + renderPath);
                    // $("*[rx-cid='" + instance.cid + "'][rx-path]").each(function (i, t) {
                    //     var $t = $(t),
                    //         keypath = $t.attr("rx-path");
                    //     if (renderPath === "" || keypath.startWith(renderPath)) {
                    //         // console.log("render:" + keypath);
                    //         instance._renderConfig(t, instance.getConfig(keypath), false);
                    //     }
                    // });
                    while (instance._renderPath.length) {
                        var path = instance._renderPath.shift();
                        // console.log("render path:"+path);
                        $("*[rx-cid='" + instance.cid + "'][rx-path='" + path + "']").each(function (i, t) {
                            instance._renderConfig(t, instance.getConfig(path), false);
                        });
                    }
                    //FixMe:————end—————触发配置渲染————Zp.20180525
                    //FixMe:———start————触发渲染后置————Zp.20180521
                    //FixMe:———start————系统声明周期事件————Zp.20180611
                    execute($options[HOOK_SYS_AFTER_RENDER], instance);
                    //FixMe:————end—————系统声明周期事件————Zp.20180611
                    execute($options[HOOK_AFTER_RENDER], instance);
                    //FixMe:————end—————触发渲染后置————Zp.20180521
                    //FixMe:———start————系统声明周期事件————Zp.20180611
                    if (afterHook && $options["_" + afterHook]) {
                        execute($options["_" + afterHook], instance);
                    }
                    //FixMe:————end—————系统声明周期事件————Zp.20180611
                    execute($options[afterHook], instance);
                    //FixMe:———start————绘制完毕自动更新历史数据————Zp.20180611
                    if (afterHook === HOOK_AFTER_MOUNT) {
                        instance.updateHistory();
                        //FixMe:———start————自适应优化执行————Zp.20180702
                        if (needResize && !instance.$parent) {
                            RX && RX.page && RX.page._autoResize();
                        }
                        //FixMe:————end—————自适应优化执行————Zp.20180702
                    }
                    //FixMe:————end—————绘制完毕自动更新历史数据————Zp.20180611
                }
            });
        };
        ...
        Rxvm.prototype.create = function (options, extra) {
            options = extend({}, options, extra);
            options.parent = this;
            var child = new Rxvm(options);
        ...
        Rxvm.prototype.importPartial = function (name) {
            return Rxvm.compile(this.partial(name));
        };
        ...
        Rxvm.prototype.compileDirective = function (directive) {
        ...
        Rxvm.prototype.destroy = function () {
            ...
            //FixMe:———start————系统声明周期事件————Zp.20180611
            execute($options[HOOK_SYS_BEFORE_DESTROY], instance);
            //FixMe:————end—————系统声明周期事件————Zp.20180611
            ...
            //FixMe:———start————系统声明周期事件————Zp.20180611
            execute($options[HOOK_SYS_AFTER_DESTROY], instance);
            //FixMe:————end—————系统声明周期事件————Zp.20180611
        ...
        Rxvm.prototype.nextTick = function (fn) {
        ...
        Rxvm.prototype.toggle = function (keypath) {
        ...
        Rxvm.prototype.increase = function (keypath, step, max) {
        ...
        Rxvm.prototype.decrease = function (keypath, step, min) {
        ...
        Rxvm.prototype.copy = function (data, deep) {
        ...
        Rxvm.prototype.insert = function (keypath, item, index) {
            //FixMe:———start————增加index为数字字符串时转换为数字值————Zp.20180815
            if(typeof index === "string"){
                var numIndex = parseInt(index,10);
                index = numIndex.isNaN() ? index : numIndex;
            }
            //FixMe:————end—————增加index为数字字符串时转换为数字值————Zp.20180815
        ...
        Rxvm.prototype.append = function (keypath, item) {
        ...
        Rxvm.prototype.prepend = function (keypath, item) {
        ...
        Rxvm.prototype.removeAt = function (keypath, index) {
            //FixMe:———start————增加index为数字字符串时转换为数字值————Zp.20180815
            if(typeof index === "string"){
                var numIndex = parseInt(index,10);
                index = numIndex.isNaN() ? index : numIndex;
        ...
        Rxvm.prototype.remove = function (keypath, item) {
         ...
         * @return {?boolean} 是否移除成功
         */
        Rxvm.prototype.removeAll = function (keypath) {
            return this.$observer.removeAll(keypath);
        };
        //FixMe:————end—————扩展方法：移除数组中的所有元素————Zp.20180628

        //FixMe:———start————扩展方法：交换顺序————Zp.20180815
        /**
         * 交换数组内两个元素的位置
         *
         * @param {string} keypath
         ...
         */
        Rxvm.prototype.exchange = function (keypath, index1, index2) {
            var list = this.get("keypath");
            if(!RX.isArray(list)){
                return false;
            }
            var num1 = parseInt(index1,10),
                num2 = parseInt(index2,10),
                listLength = list.length;
            if(num1.isNaN() || num2.isNaN() || num1 >= listLength || num2 >= listLength){
                return false;
            }
            var path1 = keypath + "." + index1,
                path2 = keypath + "." + index2,
                obj1 = RX.copy(this.get(path1),true),
                obj2 = RX.copy(this.get(path2),true);
            this.set(path1, obj2);
            this.set(path2, obj1);
            return true;
        };
        //FixMe:————end—————扩展方法：交换顺序————Zp.20180815
    ...
    Rxvm.dom = api;
    Rxvm.array = array$1;
    Rxvm.object = object$1;
    Rxvm.string = string$1;
    Rxvm.logger = logger;
    Rxvm.Event = Event;
    Rxvm.Emitter = Emitter;

    var prototype = Rxvm.prototype;
...
// 全局注册
    ...
    each([COMPONENT, 'transition', 'directive', 'partial', 'filter'], function (type) {
        prototype[type] = function (name, value) {
            ...
            if (string(name)) {
                ...
                if (length === 1) {
                    return hasValue ? data[name] : Rxvm[type](name);
                } else if (length === 2 && type === COMPONENT && func(value)) {
                    return hasValue ? getResourceAsync(data, name, value) : Rxvm[type](name, value);
                }
            }
            setResource(data || (instance[prop] = {}), name, value);
        };
        Rxvm[type] = function (name, value) {
     ...
     * @return {Array}
     */
    Rxvm.compile = function (template) {
     ...
     */
    Rxvm.validate = function (props, propTypes) {
        var result = {};
     ...
     * @param {Object} plugin
     */
    Rxvm.use = function (plugin) {
        plugin.install(Rxvm);
    };

// 全局注册内置指令
    Rxvm.directive({event: bindEvent, model: model, binding: binding});

// 全局注册内置过滤器
    Rxvm.filter({hasSlot: hasSlot});
