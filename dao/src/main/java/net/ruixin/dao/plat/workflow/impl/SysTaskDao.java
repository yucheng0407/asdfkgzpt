...
public class SysTaskDao extends BaseDao<SysTask> implements ISysTaskDao {
    ...
    public List<SysTask> findTasksByWorkflowInstance(Long id) {
        return super.findListByHql("from SysTask t where t.workflow_instance_id.id=? and " +
                "exists (select 1 from SysUser u where u.id = t.user_id.id and u.sfyxSt<>'0') order by t.id desc", id);
    }
    ...
    public List<Map<String, Object>> getSheetByNodeId(Long nodeId) {
        String sql = "SELECT NVL(NP.TITLE, P.NAME) \"name\", P.URL \"url\", N.CODE \"nodeCode\", P.CODE \"pageCode\", " +
                "       NP.CONTROL \"sheetMode\", NP.PAGE_ID \"pageId\", NP.SORT \"sId\", NP.ID \"npId\",N.SORT \"sort\" " +
    ...
    public SysNode getFirstActivityNode(String flowCode) {
        String sql = "SELECT R.END_NODE_ID\n" +
                ...
                " WHERE W.CODE = ? AND W.VERSION = (SELECT MAX(S.VERSION) FROM SYS_WORKFLOW S WHERE S.SFYX_ST = '1' AND S.CODE = W.CODE)" +
    ...
    public List<SysTask> getTaskInstanceByNode(Long wfId, Long wfInsId, String nodeDomId) {
        return super.findListByHql("select ti from SysTask ti,SysNodeInstance ni,SysNode n where\n" +
                ...
                "and n.domid=? and n.sysWorkflow.id=? and n.sfyxSt='1' \n" +
                "order by greatest(ti.cjsj,ti.accept_date,ti.finish_date) desc", wfInsId, nodeDomId, wfId);
    }
    ...
    public List<Map<String, Object>> getLatestTaskInstanceList(Long wfInstId) {
        return super.getJdbcTemplate().queryForList("SELECT G.*\n" +
                ...
                "          FROM (SELECT TI.*, NI.NODE_ID, N.DOM_ID\n" +
