define([
    ...
    declare, arrayUtils, Color, connect,webMercatorUtils,
    ...
    return declare([GraphicsLayer], {
        constructor: function(options) {
            ...
            this._levelSegmentation=options.levelSegmentation||25;
            this._zoomEnd = null;
        ...
        _setMap: function(map, surface) {
            ...
            this._zoomEnd = connect.connect(map, "onZoomEnd", this, function() {
                ...
                this.clear();
                if(map.getLevel()<this._levelSegmentation) {
                    this._clusterGraphics();
                }
        ...
        _clusterAddPoint: function(p, cluster) {
            ...
            if(cluster.attributes[p.attributes.type]){
                cluster.attributes[p.attributes.type]++;
            }else{
                cluster.attributes[p.attributes.type]=1;
            }
        ...
        _showCluster: function(c) {
            ...
            var point = webMercatorUtils.webMercatorToGeographic(latlng);
            this.add(
                new Graphic(
                    point,
                    null,
            ...
            var label = new TextSymbol(c.attributes.clusterCount.toString())
                ...
                .setOffset(0, this._clusterLabelOffset).setDecoration("none");
            this.add(
                new Graphic(
                    point,
                    label,
            ...
            var statStr="总数：";
            statStr+=c.attributes.clusterCount;
            for(x in c.attributes){
                if(x!=="clusterCount"&&x!=="clusterId"&&x!=="extent"){
                    statStr+="\n "+x.split("_")[0]+":"+c.attributes[x];
                }
            }


            var statLabel = new TextSymbol(statStr)
                .setColor(new Color([60,60,60]))
                .setFont(new Font("10pt", Font.STYLE_ITALIC,
                    Font.VARIANT_NORMAL, Font.WEIGHT_BOLD,"微软雅黑"))
                .setAlign(TextSymbol.ALIGN_START)
                .setHaloSize(3)
                .setHaloColor(new Color([255, 255, 255]))
                .setOffset(15, this._clusterLabelOffset).setDecoration("none");
            this.add(
                new Graphic(
                    point,
                    statLabel,
